...
const selectedCategory = ref(
    categoryOptions.some(
        (item) => item.value === route.query.category,
    )
...
const getImage = (product) =>
    getInsuranceImageUrl(product?.insuranceImage);
...
const loadProducts = async () => {
  ...
  try {
    productData.value =
        await insuranceRecommendationApi.getProducts(
...
const goBack = () => {
  const spendingAnalysisId = Number(
      route.query.spendingAnalysisId,
  );
