  ...
  <div class="page-layout pin-page">
    ...
    <main class="page-content pin-container">
      <header class="pin-header">
        ...
        <h1 class="text-26-bold">현재 간편비밀번호 확인</h1>
      ...
      <section class="pin-section">
        ...
        <p v-else class="security-message text-13">
    ...
    <div v-if="loading" class="loading-overlay">
      ...
      <span class="text-15-bold">간편비밀번호를 확인하고 있어요.</span>
    </div>
  </div>
</template>
...
const verifyCurrentPin = async () => {
  ...
  try {
    ...
    if (!response.verified) {
      pinPassword.value = '';
      errorMessage.value =
        response.message || '현재 간편비밀번호가 일치하지 않습니다.';
      await focusPinInput();
      return;
    }
    ...
    pinPassword.value = '';
    errorMessage.value =
      error.response?.data?.message || '현재 간편비밀번호가 일치하지 않습니다.';
