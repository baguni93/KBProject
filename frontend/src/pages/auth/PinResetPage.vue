...
// PIN 확인 화면 이동
const next = async () => {
  if (pinPassword.value.length !== 6) return;

  const result = validatePin(pinPassword.value, signupStore.phoneAuth.birthDate);
  ...
  errorMessage.value = '';
  sessionStorage.setItem('pinResetNewPin', pinPassword.value);
  router.push('/auth/pin-reset-confirm');
};
