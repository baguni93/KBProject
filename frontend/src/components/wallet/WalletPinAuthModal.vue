...
const effectivePin = computed(() => {
  const pinProp = props.inputPin || props.inputPinCode;
  return pinProp !== undefined && pinProp !== "" ? pinProp : internalPin.value;
});
...
const onInputChange = (event) => {
  ...
  emit("update:inputPinCode", cleanVal);

  const prevLen = (props.inputPin || props.inputPinCode || "").length;
  if (cleanVal.length > prevLen) {
    const lastChar = cleanVal.slice(-1);
    emit("enterPin", Number(lastChar));
  } else if (cleanVal.length < prevLen) {
    emit("deletePin");
  }
...
watch(
  ...
  (newVal) => {
    if (newVal) {
      internalPin.value = props.inputPin || props.inputPinCode || "";
      focusPinInput();
    } else {
      internalPin.value = "";
    }
